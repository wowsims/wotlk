...
func (prot *ProtectionPaladin) customRotation(sim *core.Simulation) {
	...
	if prot.GCD.IsReady(sim) {
		...
		if !prot.Rotation.UseCustomPrio {
			...
			} else if prot.HolyWrath.IsReady(sim) && (prot.Consecration.TimeToReady(sim) > time.Millisecond*6850) && (target.MobType == proto.MobType_MobTypeDemon || target.MobType == proto.MobType_MobTypeUndead) {
