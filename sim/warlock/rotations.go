...
func (warlock *Warlock) tryUseGCD(sim *core.Simulation) {
	...
	} else if warlock.Rotation.Haunt && warlock.Haunt.CD.IsReady(sim) && !warlock.HauntAura.IsActive() {
		spell = warlock.Haunt
	} else if warlock.Rotation.ChaosBolt && warlock.ChaosBolt.CD.IsReady(sim) {
