...
func (rogue *Rogue) setSubtletyBuilder() {
	...
	// Garrote
	if rogue.Garrote.Dot(rogue.CurrentTarget).IsActive() && rogue.ShadowDanceAura.IsActive() {
...
func (rogue *Rogue) setupSubtletyRotation(sim *core.Simulation) {
		...
		rogue.subtletyPrios = append(rogue.subtletyPrios, subtletyPrio{
			func(s *core.Simulation, r *Rogue) PriorityAction {
				if r.Shadowstep.IsReady(s) {
					...
					if !r.Rupture[0].Dot(r.CurrentTarget).IsActive() && rogue.ComboPoints() > 4 && rogue.CurrentEnergy() >= rogue.Rupture[1].DefaultCast.Cost+rogue.Shadowstep.DefaultCast.Cost {
						return Cast
					} else {
						return Skip
					}
	...
	if rogue.Rotation.SubtletyFinisherPriority == proto.Rogue_Rotation_Rupture {
		rogue.subtletyPrios = append(rogue.subtletyPrios, subtletyPrio{
			func(s *core.Simulation, r *Rogue) PriorityAction {
				if r.Rupture[0].Dot(r.CurrentTarget).IsActive() || s.GetRemainingDuration() < time.Second*22 {
					return Skip
				}
